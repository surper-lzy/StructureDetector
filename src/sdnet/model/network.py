...
from torchvision.models import ResNet101_Weights, resnet101   # 1. 换 backbone
...
class Network(nn.Module):
    def __init__(self, args, pretrained=True, raw_output: bool = False):
        ...
        resnet = resnet101(weights=ResNet101_Weights.DEFAULT if pretrained else None)  # 2. 换 backbone
        ...
        self.up1 = nn.Conv2d(2048, self.fpn_depth, kernel_size=1)  # 3. 512→2048
        self.up2 = Fpn(1024, self.fpn_depth)  # layer3 输出 1024
        self.up3 = Fpn(512,  self.fpn_depth)  # layer2 输出 512  (原为256)
        self.up4 = Fpn(256,  self.fpn_depth)  # layer1 输出 256
    ...
    def forward(self, x):  # (B, 3, H, W)
        ...
        p2 = self.down1(p1)  # (B, 256, H/4, W/4)
        p3 = self.down2(p2)  # (B, 512, H/8, W/8)
        p4 = self.down3(p3)  # (B,1024, H/16,W/16)
        p5 = self.down4(p4)  # (B,2048, H/32,W/32)
